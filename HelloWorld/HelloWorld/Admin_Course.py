...
def courseinfomodify(request):
    ...
    try:
        if request.method == 'POST':
            ...
            updatecourse = Course.objects.filter(cNo=_id)
            updatecourseteacher = TeacherCourse.objects.filter(cNo=_id, tNo=_formerteacherid)
            checkformerteacherid = Teacher.objects.filter(tNo=_formerteacherid)
            checknewteacherid = Teacher.objects.filter(tNo=_newteacherid)
            checknewcourseteacherid = TeacherCourse.objects.filter(cNo_id=_id, tNo_id=_newteacherid)
                    ...
                    if len(updatecourse) == 1:
                        ...
                        if len(checknewcourseteacherid) == 0:
                            updatecourse.update(cName=_name, cCredit=_credit, cNature=_nature, cNumber=_number,
                                                cMajor=_major, cGrade=_grade, cTerm=_term, cComposition=_composition)
                            updatecourseteacher.update(tNo=_newteacherid)
                            info = "Success"
