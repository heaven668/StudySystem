                    ...
                    for cl in courselist:
                        if len(TeacherCourse.objects.filter(cNo_id=cl.cNo)) > 0:
                            ...
                            for tcs in tc:
                                ...
                                break
                            sc = StudentCourse.objects.filter(cNo=cl.cNo, sNo=_id)
                            if len(sc) == 0:
                                c['status'] = "0"
                            else:
                                c['status'] = "1"
    ...
    try:
        if request.method == "POST":
            ...
            if time.time() - time.mktime(Token.objects.get(key=_token).created.timetuple()) > AuthorizedTime:
                ...
                elif len(StudentCourse.objects.filter(cNo_id=int(_id), sNo_id=_stu_id, status=1)) != 0:
                    info = "You have selected the course"
                else:
                    selectcourse = StudentCourse(cNo_id=int(_id), sNo_id=_stu_id, status=1)
                    selectcourse.save()
                    info = "Success"
            else:
                info = "Student not exist"
